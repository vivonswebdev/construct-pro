...
export async function seedDataIfEmpty(companyId: string): Promise<boolean> {
  const { data: locked, error: lockErr } = await supabase.rpc("seed_lock", {
  ...
  const clientsSeed = [
    {
      type: "entreprise",
      raison_sociale: "Immobilière Dumont SA",
      nom: "Dumont",
      numero_bce: "0123.456.789",
      numero_tva: "BE0123456789",
      assujetti_tva: true,
      adresse: "Rue des Acacias 14",
      code_postal: "4000",
      ville: "Liège",
    ...
    {
      type: "entreprise",
      raison_sociale: "TechLog BV",
      nom: "Janssens",
      numero_bce: "0456.789.012",
      numero_tva: "BE0456789012",
      assujetti_tva: true,
      adresse: "Industrieweg 8",
      code_postal: "3500",
      ville: "Hasselt",
    ...
    {
      type: "entreprise",
      raison_sociale: "ASBL Les Jardins du Cœur",
      nom: "Lambert",
      numero_bce: "0678.901.234",
      numero_tva: null,
      assujetti_tva: false,
    ...
    },
    {
      ...
      prenom: "Luc",
      nom: "Pirard",
      adresse: "Avenue de la Citadelle 22",
      code_postal: "5000",
      ville: "Namur",
  ...
  const chantiersData = [
    ...
    {
      client: cl[4],
      name: "Extension maison Collignon",
      budget: 86000,
      actual_costs: 31200,
    ...
    {
      client: cl[1],
      name: "Entrepôt Logistique Hasselt",
      budget: 180000,
      actual_costs: 128200,
  ...
  if (vehicules && vehicules.length >= 7) {
    await supabase.from("vehicule_affectations").insert([
      ...
      },
      {
        ...
        chantier_id: chantiers[1].id,
        start_date: inDays(-35),
        start_km: 110900,
      },
      {
        vehicule_id: vehicules[3].id,
        chantier_id: chantiers[2].id,
      ...
      },
      {
        ...
        chantier_id: chantiers[3].id,
        start_date: inDays(-190),
        end_date: inDays(-25),
        start_km: 21000,
        end_km: 27900,
      },
  ...
  for (const w of contractWorkers) {
    ...
    salaryRows.push({
      company_id: companyId,
      personnel_id: w.id,
      period_month: currentM,
      period_year: currentY,
      gross_amount: gross,
      net_amount: net,
      paid: false,
    });
  ...
  const devisSeed: {
  ...
  }[] = [
    {
      ...
      autoliq: true,
      lignes: [
        ["Terrassement et évacuation des terres", 850, 18, 0],
        ["Fondations en béton armé", 1, 38500, 0],
        ["Gros œuvre maçonnerie (blocs béton)", 1, 112000, 0],
        ["Charpente et toiture tuiles", 1, 54500, 0],
        ["Menuiseries extérieures PVC", 1, 40000, 0],
      ],
    },
    {
      client: cl[4],
      ...
      valid: -25,
      lignes: [
        ["Terrassement et fondations", 1, 9800, 21],
        ["Gros œuvre extension 45 m²", 45, 950, 21],
        ["Toiture plate EPDM + isolant PUR", 45, 185, 21],
        ["Baie vitrée alu 4 m", 1, 6450, 21],
        ["Raccordements et finitions", 1, 18500, 21],
      ],
    },
    {
      client: cl[1],
      ...
      autoliq: true,
      lignes: [
        ["Dalle industrielle béton 1 500 m²", 1500, 62, 0],
        ["Structure acier et bardage", 1, 58000, 0],
        ["Toiture bac acier isolée", 1, 29000, 0],
      ],
    },
    {
      client: cl[3],
      ...
      att6: true,
      lignes: [
        ["Démolition et évacuation", 1, 6200, 6],
        ["Remplacement toiture complète", 180, 145, 6],
        ["Isolation murs par l'intérieur", 220, 48, 6],
        ["Électricité mise en conformité RGIE", 1, 9800, 6],
        ["Peinture et finitions", 1, 12040, 6],
      ],
    },
    {
      client: cl[5],
      ...
      att6: true,
      lignes: [
        ["Rénovation salle de bain complète", 1, 14500, 6],
        ["Remplacement châssis double vitrage", 6, 1150, 6],
        ["Carrelage sol 35 m²", 35, 72, 6],
      ],
    },
    {
      client: cl[2],
      ...
      valid: 28,
      lignes: [
        ["Terrassement salle polyvalente", 1, 12500, 21],
        ["Fondations et dalle", 1, 34000, 21],
        ["Gros œuvre et toiture", 1, 71000, 21],
        ["Techniques spéciales (HVAC)", 1, 14500, 21],
      ],
    },
    {
      client: cl[4],
      ...
      valid: -40,
      lignes: [
        ["Aménagement abri de jardin", 1, 7800, 21],
        ["Terrasse bois exotique 30 m²", 30, 165, 21],
      ],
    },
    {
      client: cl[1],
      ...
      autoliq: true,
      lignes: [
        ["Extension bureaux mezzanine", 120, 480, 0],
        ["Cloisons plaques de plâtre", 180, 42, 0],
      ],
    },
  ];
  const yr4 = today.getFullYear();
  for (const [i, d] of devisSeed.entries()) {
    const lignes = d.lignes.map(([description, quantity, unit_price, vat]) => ({
      description,
      quantity,
      unit_price,
      vat_rate: d.autoliq ? 0 : (vat ?? 21),
    }));
    const subtotal = lignes.reduce((s, l) => s + l.quantity * l.unit_price, 0);
    const vatAmount = lignes.reduce(
      (s, l) => s + (l.quantity * l.unit_price * l.vat_rate) / 100,
      0,
    );
    ...
    const { data: inserted } = await supabase
      ...
      .insert({
        company_id: companyId,
        type: "devis",
        number: `DEV-${yr4}-${String(i + 1).padStart(4, "0")}`,
        status: d.status,
        client_id: d.client?.id ?? null,
        client_name: cName(d.client) ?? "Client",
        client_address: cAddr(d.client),
        client_vat: d.client?.numero_tva ?? null,
        chantier_id: d.chantier?.id ?? null,
        issue_date: inDays(d.issue),
        valid_until: inDays(d.valid),
        due_date: inDays(d.valid),
        vat_rate: mainRate,
  ...
  const matSeed = [
    {
      name: "Ciment Portland CEM I 42.5N",
      sku: "CIM-425",
      unit: "sac 25kg",
      unit_price: 6.5,
      stock_quantity: 120,
    ...
    },
    {
      ...
      stock_quantity: 12,
      min_stock: 5,
      supplier: "Carrières du Hainaut",
      category: "Granulats",
    },
    {
      name: "Gravier concassé 4/14",
      sku: "GRV-414",
      unit: "m³",
      unit_price: 42,
      stock_quantity: 9,
    ...
    {
      name: "Chevron sapin 63x175 (5m)",
      sku: "CHV-63",
      unit: "pièce",
      unit_price: 18.9,
      stock_quantity: 36,
      min_stock: 15,
      supplier: "Van Hoorebeke",
      category: "Bois",
    },
    {
      name: "Plaque OSB3 18mm",
      sku: "OSB-18",
      unit: "panneau",
      unit_price: 32,
      stock_quantity: 40,
  ...
  if (materiaux && materiaux.length) {
    ...
    const mv = (
      ...
      return {
        company_id: companyId,
        materiau_id: mat.id,
        chantier_id: chIdx === null ? null : chantiers[chIdx].id,
        type,
        quantity,
        unit_price: mat.unit_price,
        total: quantity * Number(mat.unit_price),
        date: inDays(day),
        ...extra,
      };
    };
