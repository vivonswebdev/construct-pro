...
export function ClientFormModal({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (f.assujetti_tva && !f.numero_tva) return toast.error("Un client assujetti doit avoir un numéro de TVA");
    setSaving(true);
    const payload = {
      ...f,
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <form onSubmit={submit} onClick={(e) => e.stopPropagation()} className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-card p-6 shadow-xl">
        ...
        <div className="grid grid-cols-2 gap-3">
          {isEnt ? (
            <>
              <L label="Raison sociale *" full><input className={inputCls} value={f.raison_sociale} onChange={(e) => set({ raison_sociale: e.target.value })} placeholder="Ex: Dumont Immo SRL" /></L>
              <L label="N° BCE"><input className={inputCls} value={f.numero_bce} onChange={(e) => set({ numero_bce: e.target.value })} placeholder="0123.456.789" /></L>
              <L label="Contact (nom)"><input className={inputCls} value={f.nom} onChange={(e) => set({ nom: e.target.value })} /></L>
            </>
          ) : (
            <>
              <L label="Prénom"><input className={inputCls} value={f.prenom} onChange={(e) => set({ prenom: e.target.value })} /></L>
              <L label="Nom *"><input className={inputCls} value={f.nom} onChange={(e) => set({ nom: e.target.value })} /></L>
            </>
          )}
          <L label="N° TVA">
            <input className={`${inputCls} ${vatInvalid ? "border-red-500" : ""}`} value={f.numero_tva} onChange={(e) => set({ numero_tva: e.target.value })} placeholder="BE0123456789" />
            {vatInvalid && <span className="mt-1 block text-xs text-red-600">Format invalide</span>}
          </L>
          <label className="flex items-end gap-2 pb-2 text-sm">
            <input type="checkbox" checked={f.assujetti_tva} onChange={(e) => set({ assujetti_tva: e.target.checked })} />
            Assujetti à la TVA
          </label>
          <L label="Adresse" full><input className={inputCls} value={f.adresse} onChange={(e) => set({ adresse: e.target.value })} /></L>
          <L label="Code postal"><input className={inputCls} value={f.code_postal} onChange={(e) => set({ code_postal: e.target.value })} /></L>
          <L label="Ville"><input className={inputCls} value={f.ville} onChange={(e) => set({ ville: e.target.value })} /></L>
          <L label="Email"><input type="email" className={inputCls} value={f.email} onChange={(e) => set({ email: e.target.value })} /></L>
          <L label="Téléphone"><input className={inputCls} value={f.telephone} onChange={(e) => set({ telephone: e.target.value })} /></L>
          <L label="Langue">
            <select className={inputCls} value={f.langue} onChange={(e) => set({ langue: e.target.value as "FR" | "NL" })}>
              <option value="FR">Français</option><option value="NL">Nederlands</option>
            </select>
          </L>
          <L label="Notes" full><textarea rows={2} className={inputCls} value={f.notes} onChange={(e) => set({ notes: e.target.value })} /></L>
        </div>
