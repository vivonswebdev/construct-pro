...
import { formatEURBE, quarterOfMonth, onssDueDate, precompteDueDate, DEFAULTS, MONTHS_FR } from "@/lib/belgian";
...
function PrecomptePage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [pers, sal, prec, onss, aff] = await Promise.all([
        ...
        supabase.from("salary_payments").select("*").eq("company_id", companyId!).eq("period_year", period.year).eq("period_month", period.month + 1),
        supabase.from("precompte_payments").select("*").eq("company_id", companyId!).eq("period_year", period.year).eq("period_month", period.month + 1),
        supabase.from("onss_payments").select("*").eq("company_id", companyId!).eq("year", period.year).eq("quarter", quarter),
        supabase.from("affectations").select("personnel_id, chantiers(name)").is("end_date", null),
      ]);
      return {
        personnel: (pers.data ?? []).filter((p) => ["CDI", "CDD", "Intérim"].includes(p.contract_type ?? "")),
        salaries: sal.data ?? [],
  ...
  const stats = useMemo(() => {
    ...
    const unpaid = rows.reduce((s, r) => {
      let n = 0;
      if (!r.salary.row?.paid) n += r.salary.row ? Number(r.salary.row.net_amount ?? 0) : r.salary.defaultNet;
      if (!r.precompte.row?.paid) n += r.precompte.row ? Number(r.precompte.row.amount ?? 0) : r.precompte.defaultAmount;
      if (!r.onss.row?.paid) n += r.onss.row ? Number(r.onss.row.amount ?? 0) : r.onss.defaultAmount;
      return s + n;
    }, 0);
    return { salPaid, precPaid, onssPaid, total: rows.length, totalUnpaid: unpaid, anyOnssPaid: onssPaid > 0 && onssPaid === total };
  }, [rows]);
  ...
  const confirmPayment = async (paidDate: string, reference: string, amount: number) => {
    ...
    if (kind === "salary") {
      ...
      const res = await supabase.from("salary_payments").upsert({
        ...
        period_month: period.month + 1, period_year: period.year,
        gross_amount: gross, net_amount: amount,
        paid: true, paid_date: paidDate, reference, payment_method: "Virement",
      ...
      const res = await supabase.from("precompte_payments").upsert({
        company_id: companyId, personnel_id: personnelId,
        period_month: period.month + 1, period_year: period.year,
        amount, paid: true, paid_date: paidDate, reference,
  ...
  const cancelPayment = async (kind: PayKind, rowId: string) => {
    const table = kind === "salary" ? "salary_payments" : kind === "precompte" ? "precompte_payments" : "onss_payments";
    const { error } = await supabase.from(table).update({ paid: false, paid_date: null, reference: null }).eq("id", rowId);
    if (error) toast.error(error.message);
    else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Suivi des obligations sociales et fiscales par ouvrier</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setPeriod((p) => ({ year: p.month === 0 ? p.year - 1 : p.year, month: p.month === 0 ? 11 : p.month - 1 }))}
          ...
          <button
            onClick={() => setPeriod((p) => ({ year: p.month === 11 ? p.year + 1 : p.year, month: p.month === 11 ? 0 : p.month + 1 }))}
            className="rounded-md border border-border p-1.5 hover:bg-muted"
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
        <KpiCard label="Salaires payés" value={`${stats.salPaid}/${stats.total}`} tone={stats.salPaid === stats.total ? "success" : stats.salPaid > 0 ? "warning" : "danger"} />
        <KpiCard label="Précompte payé" value={`${stats.precPaid}/${stats.total}`} tone={stats.precPaid === stats.total ? "success" : stats.precPaid > 0 ? "warning" : "danger"} />
        <KpiCard label={`ONSS Q${quarter}/${period.year}`} value={stats.anyOnssPaid ? "Payé" : `${stats.onssPaid}/${stats.total}`} tone={stats.anyOnssPaid ? "success" : "danger"} />
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        <div className="border-b border-border px-5 py-3">
          <h2 className="text-base font-semibold">Ouvriers actifs — {MONTHS_FR[period.month]} {period.year}</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 && (
                <tr><td colSpan={8} className="px-3 py-8 text-center text-sm text-muted-foreground">Aucun ouvrier actif (CDI/CDD/Intérim).</td></tr>
              )}
              {rows.map((r) => {
                  ...
                  <tr key={r.person.id} className="border-b border-border/50">
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <div className={`flex h-8 w-8 items-center justify-center rounded-full text-[10px] font-bold text-white ${avatarColor(r.person.full_name)}`}>
                    ...
                    <td className="px-3 py-2.5 text-right font-mono text-xs">{formatEURBE(r.salary.row ? Number(r.salary.row.net_amount) : r.salary.defaultNet)}</td>
                    <td className="px-3 py-2.5">
                      <PayStatusCell
                        paid={!!r.salary.row?.paid}
                        paidDate={r.salary.row?.paid_date}
                        onPay={() => setPayModal({ kind: "salary", personName: r.person.full_name, defaultAmount: r.salary.row ? Number(r.salary.row.net_amount) : r.salary.defaultNet, personnelId: r.person.id, existingId: r.salary.row?.id })}
                        onCancel={() => r.salary.row && cancelPayment("salary", r.salary.row.id)}
                      />
                    </td>
                    <td className="px-3 py-2.5 text-right font-mono text-xs">{formatEURBE(r.precompte.row ? Number(r.precompte.row.amount) : r.precompte.defaultAmount)}</td>
                    <td className="px-3 py-2.5">
                      <PayStatusCell
                        paid={!!r.precompte.row?.paid}
                        paidDate={r.precompte.row?.paid_date}
                        onPay={() => setPayModal({ kind: "precompte", personName: r.person.full_name, defaultAmount: r.precompte.row ? Number(r.precompte.row.amount) : r.precompte.defaultAmount, personnelId: r.person.id, existingId: r.precompte.row?.id })}
                        onCancel={() => r.precompte.row && cancelPayment("precompte", r.precompte.row.id)}
                      />
                    </td>
                    <td className="px-3 py-2.5 text-right font-mono text-xs">{formatEURBE(r.onss.row ? Number(r.onss.row.amount) : r.onss.defaultAmount)}</td>
                    <td className="px-3 py-2.5">
                      <PayStatusCell
                        paid={!!r.onss.row?.paid}
                        paidDate={r.onss.row?.paid_date}
                        onPay={() => setPayModal({ kind: "onss", personName: r.person.full_name, defaultAmount: r.onss.row ? Number(r.onss.row.amount) : r.onss.defaultAmount, personnelId: r.person.id, existingId: r.onss.row?.id })}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={() => toast.info("Export PDF bientôt disponible")} className="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-3 py-1.5 text-xs font-semibold hover:bg-muted">
          <FileDown className="h-3.5 w-3.5" /> Exporter PDF — Récapitulatif
        </button>
        <button onClick={() => toast.info("Export CSV bientôt disponible")} className="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-3 py-1.5 text-xs font-semibold hover:bg-muted">
          <FileDown className="h-3.5 w-3.5" /> Exporter CSV
        </button>
        <button disabled className="inline-flex cursor-not-allowed items-center gap-1.5 rounded-md border border-border bg-muted px-3 py-1.5 text-xs font-semibold text-muted-foreground">
...
function RateInput({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <div>
      <label className="mb-1 block text-[11px] font-semibold uppercase text-muted-foreground">{label}</label>
...
function KpiCard({ label, value, tone }: { label: string; value: string; tone: "success" | "warning" | "danger" }) {
  const cls = tone === "success" ? "border-l-emerald-500 bg-emerald-50/40"
    : tone === "warning" ? "border-l-amber-500 bg-amber-50/40"
    : "border-l-red-500 bg-red-50/40";
  const text = tone === "success" ? "text-emerald-700" : tone === "warning" ? "text-amber-700" : "text-red-700";
  return (
...
function AlertBar({ tone, text }: { tone: "warning" | "danger"; text: string }) {
  const cls = tone === "danger" ? "border-red-200 bg-red-50 text-red-700" : "border-amber-200 bg-amber-50 text-amber-700";
  return (
...
function PayStatusCell({ paid, paidDate, onPay, onCancel }: { paid: boolean; paidDate?: string | null; onPay: () => void; onCancel: () => void }) {
  if (paid) {
    return (
      <div className="flex flex-col gap-0.5">
        ...
        <button onClick={onCancel} className="w-fit text-[10px] text-muted-foreground hover:text-red-600 hover:underline">Annuler</button>
      </div>
    );
  }
  return (
    <button onClick={onPay} className="rounded-md border border-primary px-2 py-1 text-xs font-semibold text-primary hover:bg-primary hover:text-white">
      Payer
    </button>
  );
}
...
function PayModal({
  ...
  const title = modal.kind === "salary" ? "Paiement du salaire" : modal.kind === "precompte" ? "Paiement du précompte" : "Paiement ONSS";
  const note = modal.kind === "precompte"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-card p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Ouvrier : <span className="font-medium text-foreground">{modal.personName}</span></p>

        <div className="mt-4 space-y-3">
          <Field label="Montant (€)">
            <input type="number" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm" />
          </Field>
          <Field label="Date de paiement">
            <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm" />
          </Field>
          <Field label="Référence virement">
            <input type="text" value={reference} onChange={(e) => setReference(e.target.value)} placeholder="Ex: VIR-2025-06-001" className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm" />
          </Field>
          {note && <p className="rounded-md bg-amber-50 px-3 py-2 text-[11px] text-amber-700">ⓘ {note}</p>}
        </div>

        <div className="mt-5 flex justify-end gap-2">
          <button onClick={onClose} className="rounded-lg border border-border px-4 py-2 text-sm font-semibold hover:bg-muted">Annuler</button>
          <button onClick={() => onConfirm(date, reference, parseFloat(amount) || 0)} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary/90">
