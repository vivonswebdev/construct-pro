...
function PersonnelDetail() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [pRes, presRes, affRes] = await Promise.all([
        ...
        supabase.from("presence").select("*").eq("personnel_id", id).gte("date", monthStart).lte("date", monthEnd),
  ...
  const togglePresence = async (date: string, currentStatus: string | null) => {
    const next = currentStatus === "Présent" ? "Absent" : currentStatus === "Absent" ? "Congé" : "Présent";
    const { error } = await supabase
      .from("presence")
      .upsert({ personnel_id: id, date, status: next, hours: next === "Présent" ? 8 : 0 }, { onConflict: "personnel_id,date" });
    if (error) toast.error(error.message);
    else qc.invalidateQueries({ queryKey: ["personnel-detail"] });
  };

  const monthName = new Date(viewMonth.year, viewMonth.month, 1).toLocaleDateString("fr-BE", { month: "long", year: "numeric" });
  const firstDay = new Date(viewMonth.year, viewMonth.month, 1);
  const startOffset = (firstDay.getDay() + 6) % 7; // Mon=0
  const daysInMonth = new Date(viewMonth.year, viewMonth.month + 1, 0).getDate();
  const today = new Date(); today.setHours(0, 0, 0, 0);

  const cells: { date?: string; day?: number; dow?: number; future?: boolean; isToday?: boolean }[] = [];
  ...
  return (
    <div className="space-y-6">
      <Link to="/personnel" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
        <div className="flex flex-wrap items-center gap-4">
          <div className={`flex h-16 w-16 items-center justify-center rounded-full text-lg font-bold text-white ${avatarColor(person.full_name)}`}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              ...
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${person.status === "Actif" ? "bg-emerald-100 text-emerald-700" : "bg-gray-200 text-gray-600"}`}>
                {person.status}
              </span>
            </div>
          </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <InfoItem icon={Euro} label="Taux horaire" value={person.hourly_rate ? `${person.hourly_rate} €/h` : "—"} />
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
        ...
        <div className="grid grid-cols-7 gap-1.5 text-center text-xs">
          ...
          {cells.map((c, i) => {
            ...
            else if (pr?.status === "Présent") { style = "bg-emerald-100 text-emerald-700"; label += " ✓"; }
            else if (pr?.status === "Absent") { style = "bg-red-100 text-red-600"; label += " ✗"; }
            else if (pr?.status === "Congé") { style = "bg-amber-100 text-amber-600"; label += " ~"; }
...
function Stat({ value, label, tone, suffix }: { value: number; label: string; tone: string; suffix?: string }) {
  return (
    <div className="rounded-lg bg-muted/50 px-3 py-2">
      <span className={`font-bold ${tone}`}>{value}{suffix ?? ""}</span>{" "}
